...
st.subheader("Оценка остаточного риска ИБ, защитных затрат и возможных правовых последствий для малого и среднего бизнеса")

st.markdown("""
Модель считает **остаточный риск после защитных затрат**.  
ИБ не трактуется как инвестиционный актив: учитываются плановые затраты на предотвращение,
обнаружение, соблюдение норм права, реагирование и восстановление.
""")
...
st.subheader("6. Четвёртый слой: социо-правовой риск штрафов, жалоб и публичности")
lm1, lm2, lm3, lm4 = st.columns(4)
lm1.metric("Социо-правовой разрыв", f"{fine_summary['socio_legal_gap_score']:.1f}%")
...
st.caption("""
Социо-правовой слой отделён от технических атак: он отражает стоимость несоблюдения норм,
жалобы субъектов данных, публичность инцидента, реакцию регулятора и штрафной хвост.
Это не юридическое заключение и не прогноз судебного решения.
""")

with st.expander("Таблица возможных штрафных сценариев", expanded=True):
    if not has_pdn:
        st.info("Социо-правовой штрафной блок не активирован: персональные данные не отмечены.")
    else:
        st.dataframe(
            fine_df[["Сценарий штрафа", "Норма", "Вероятностный коэффициент", "Штраф минимум, руб.", "Штраф максимум, руб.", "Ожидаемый штраф средний, руб.", "Условие"]],
            use_container_width=True
        )

if not layers.empty:
    st.subheader("7. Остаточный риск по четырём слоям")
    st.altair_chart(
        alt.Chart(layers).mark_bar().encode(
            x=alt.X("Остаточный риск, %:Q", title="Остаточный риск, %"),
            y=alt.Y("Слой:N", sort="-x", title="Слой"),
            tooltip=list(layers.columns)
...
if risk_df.empty:
    ...
    st.altair_chart(
        alt.Chart(top).mark_bar().encode(
            ...
            tooltip=["Угроза / правовой разрыв", "Слой", "Исходный риск, руб.", "Остаточный риск, руб.", "Применённые меры"]
        ).properties(height=360),
        use_container_width=True
    )
    st.dataframe(
        risk_df[["Угроза / правовой разрыв", "Слой", "Вероятность", "Уязвимость", "Оценка ущерба, руб.", "Исходный риск, руб.", "Остаточный риск, руб.", "Остаточный риск, %", "Применённые меры"]].sort_values("Остаточный риск, руб.", ascending=False),
        use_container_width=True
    )
...
3. Социо-правовой слой требует актуализации по действующей редакции закона и практике.
4. Защитные меры учитываются через качество внедрения, а не через факт оплаты.
5. В модели не используется ROI; анализируются только затраты, остаточный риск и предотвращаемый ущерб.
""")
